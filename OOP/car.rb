...
class Car
  ...
  def initialize(speed = 0, engine_volume = 1.6) # У нас могут быть разные параметры, нам нужна возможность управлять объектом, с какими параметрами он будет создан. Для этого существуют параметры методов и конструкторов. Использовать можем как локальные переменные.
    @speed = speed
