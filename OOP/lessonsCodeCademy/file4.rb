...
Когда хорошего мало
С другой стороны, иногда, работая с производным классом (или подклассом), вы понимаете, что переписали метод или атрибут,
определенный в базовом классе этого класса (также называемом родительским или суперклассом),
который вам действительно нужен. Не бойтесь! Вы можете получить прямой доступ к атрибутам или методам суперкласса
с помощью встроенного в Ruby ключевого слова super.
...
Когда вы вызываете super изнутри метода, это указывает Ruby на необходимость поиска в суперклассе текущего класса метода
с тем же именем, что и метод, из которого вызывается super. Если он найден, то Ruby будет использовать версию метода
суперкласса.
=end
